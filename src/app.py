...
class App:
    def __init__(self, topology, n_servers=sample_data['app_n_servers'], n_clients=sample_data['app_n_clients'], 
                 ...
                 app_max_packets=sample_data['app_max_packets'], app_interval=sample_data['app_interval'], app_packet_size=sample_data['app_packet_size'], app_start_time=sample_data['app_start_time'],
                 app_duration=sample_data['app_duration'],tcp_app_data_rate=sample_data['tcp_app_data_rate'] ,app_port=sample_data['app_port'],animFile=sample_data['app_animation_file']):
    ...
    def setup_client(self, client, server):
        ...
        if self.app_type == "udp_echo":
            ...
            echo_client.SetAttribute(
                "DataRate", ns.DataRateValue(ns.DataRate(self.tcp_app_data_rate)))
            echo_client.SetAttribute("PacketSize", ns.UintegerValue(self.app_packet_size))
