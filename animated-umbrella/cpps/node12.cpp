...
std::ofstream node12_packetLogFile("animated-umbrella/cpps/packets_log.txt", std::ios::out | std::ios::app);
...
void node12_RxCallback(Ptr<const Packet> packet, Ptr<Ipv4> ipv4,uint32_t interfaceIndex) {
    ...
    // Store packet info
    PacketInfo12 pktInfo = { packet->GetUid(), packetType, destPort, time, packet->GetSize(),offset };
    node12_receivedPackets.push_back(pktInfo);

    // Log the packet to the file
    if (node12_packetLogFile.is_open()) {
        node12_packetLogFile << "[Node " << node12_src << "] Packet: " << packet->GetUid()
                              << ", RX: " << packetType
                              << ", Port: " << destPort
                              << ", Time: " << time
                              << ", Size: " << packet->GetSize()
...
void node12_TxCallback(Ptr<const Packet> packet, Ptr<Ipv4> ipv4,uint32_t interfaceIndex) {
    Ipv4Header ipHeader;
    UdpHeader udpHeader;
    TcpHeader tcpHeader;
    ...
    // Store packet info
    PacketInfo12 pktInfo = { packet->GetUid(), packetType, destPort, time, packet->GetSize(),offset };
    node12_transmittedPackets.push_back(pktInfo);

    // Log the packet to the file
    if (node12_packetLogFile.is_open()) {
        node12_packetLogFile << "[Node " << node12_src << "] Packet: "
                              << packet->GetUid()
                              << ", TX: " << packetType
                              << ", Port: " << destPort
                              << ", Time: " << time
...
void node12_ClosePacketLog() {
    if (node12_packetLogFile.is_open()) {
        node12_packetLogFile.close();
    }
}

// Node-specific exported functions without C linkage
Callback<void, Ptr<const Packet>, Ptr<Ipv4>, uint32_t> node12_CreateRxCallback() {
    return MakeCallback(&node12_RxCallback);
}

Callback<void, Ptr<const Packet>, Ptr<Ipv4>, uint32_t> node12_CreateTxCallback() {
    return MakeCallback(&node12_TxCallback);
}

// Data access functions
std::vector<PacketInfo12> node12_GetTransmittedPackets() {
    return node12_transmittedPackets;
}

std::vector<PacketInfo12> node12_GetReceivedPackets() {
    return node12_receivedPackets;
}
